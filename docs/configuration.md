...
| `auth.json` | YouTube Music browser-cookie credentials (auto-generated by `ytm setup`) |
| `oauth.json` | YouTube Music OAuth token (auto-generated by `ytm setup --oauth`; see [docs/oauth-login.md](oauth-login.md)) |
| `oauth_creds.json` | Your Google OAuth client ID/secret, saved after the first `ytm setup --oauth` run so you don't re-enter them |
