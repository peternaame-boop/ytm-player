...
- **Playback no longer risks an unauthenticated `android_vr` 403** — client selection moved from `android` (silently dropped by yt-dlp once cookies are present, and not actually relevant to the "made for kids" content it was chosen for — see project notes) to `web_safari`/`web_embedded`, both of which support cookie-based auth. yt-dlp's own default client selection — which could otherwise fall back to `android_vr` — is never used now that the client list is always explicit, for every setup type. Separately, stream resolution now reads the cookie data `ytm setup` (browser-based *or* manual header paste — both are parsed into a stream cookiejar the same way) or its automatic session refresh already produced, instead of independently detecting a browser itself.
- **Stream format selection now prioritizes HLS and drops the audio-only fallback** — `QUALITY_FORMATS` switched from a `bestaudio`-first selector to a combined `[vcodec!=none][acodec!=none]` selector with HLS (`[protocol^=m3u8]`) prioritized within each quality tier; `bestaudio` is dropped entirely, not just deprioritized, because it's PO-Token-gated regardless of client or cookies. Per @Villoh's PR #136 testing: the combined+HLS format with cookies hit 24/24 (100%) success vs. 4/12 (33%) for the prior `bestaudio`-based selector. Every quality tier now fetches a muxed audio+video stream — but since `ytm-player` never renders video (`video=False`) and video resolution has no bearing on the audio track's own encoding, each tier's format string now leads with a `[height<=144]` alternative to minimize the wasted video bandwidth at no cost to audio quality, falling back to the exact validated (unconstrained) chain for any video with no low-res combined+HLS rendition available. The `[height<=144]` alternative deliberately stands alone, never bracketed together with an `abr` filter: yt-dlp reports `abr=None` for every combined format extracted this way, so an `[height<=144][abr<=N]` alternative would never match anything and the height cap would silently stop working. The pre-existing `abr<=128`/`abr<=64` alternatives further down each tier's chain are consequently inert today (kept only as harmless forward-compat in case yt-dlp ever starts populating `abr` for combined formats) — quality tiers currently have no way to differentiate actual audio bitrate on combined YouTube formats; the `high`/`medium`/`low` distinction now mainly affects fallback behavior on videos without a low-resolution combined+HLS rendition.
