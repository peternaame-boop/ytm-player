...
- **Stream format selection now prioritizes HLS and drops the audio-only fallback** — `QUALITY_FORMATS` switched from a `bestaudio`-first selector to a combined `[vcodec!=none][acodec!=none]` selector with HLS (`[protocol^=m3u8]`) prioritized within each quality tier; `bestaudio` is dropped entirely, not just deprioritized, because it's PO-Token-gated regardless of client or cookies. Per @Villoh's PR #136 testing: the combined+HLS format with cookies hit 24/24 (100%) success vs. 4/12 (33%) for the prior `bestaudio`-based selector. Trade-off: every quality tier now fetches a muxed audio+video stream — mpv discards the video (`video=False`) rather than not downloading it, so this trades some bandwidth for the large reliability gain; the `abr` limits on the "low"/"medium" tiers still cap audio bitrate but no longer bound the total download the way they did when audio-only was the only stream fetched.
