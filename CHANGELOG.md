...
- **Streaming uses your `ytm setup` session** — the stream resolver now loads the youtube.com/google.com cookies that `ytm setup` (browser or manual header paste) and the automatic session refresh write to `stream_cookies.txt`, so tracks that need your account (Premium-only, age-restricted) resolve as you instead of anonymously. It never decrypts browser cookies on its own; without a setup session it streams anonymously as before. A cookie file rewritten mid-session by an auto-refresh is picked up on the next resolve, and yt-dlp never writes the file back. The format fallback now requires a stream with an audio track. Thanks @wgordon17 (#136).
