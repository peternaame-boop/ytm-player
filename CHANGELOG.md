...
- **Six concurrency bugs in stream resolution, all confirmed in practice**: a UI freeze from closing a live `YoutubeDL` instance out from under an in-flight resolve; a reset that landed while a slow options-build was in flight being silently lost, failing every subsequent track identically; a lock held across the entire (slow, Keychain-backed) cookie-decryption step blocking the UI thread for its full duration; a second, separate UI freeze from deciding whether to show the "setting up playback" toast, which used to block on that same slow decryption if one was already running elsewhere; duplicate "setting up playback" toasts when the startup warmup and a direct play action raced each other; and two concurrent resolves both independently re-decrypting the same browser's entire cookie store.
...
- **Hardened the new stream-cookie cache file** — it's now written with the same symlink-safe `O_NOFOLLOW` open already used for `auth.json`, and scoped to `youtube.com`/`google.com` cookies only instead of the full browser cookie jar (every site the browser has a cookie for).
