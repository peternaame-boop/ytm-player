...
- **Streaming can use your `ytm setup` session** — opt in with `[yt_dlp] use_session_cookies = true` and the stream resolver loads the youtube.com/google.com cookies that `ytm setup` (browser or manual header paste) and the automatic session refresh write to `stream_cookies.txt`, so tracks that need your account (Premium-only, age-restricted) resolve as you. This mode needs yt-dlp's JavaScript challenge solver (`remote_components = "ejs:github"` plus a JS runtime); see [Configuration](docs/configuration.md#yt_dlp). Off by default: streaming stays anonymous and needs nothing extra. The resolver never decrypts browser cookies on its own, a cookie file rewritten mid-session by an auto-refresh is picked up on the next resolve, and yt-dlp never writes the file back. The format fallback now requires a stream with an audio track. Thanks @wgordon17 (#136).
