...
- **`J`/`K` on a sorted Queue page clear the sort first** — the move happened in queue order while the sorted view stayed put, so nothing appeared to happen. The table now shows queue order and the row moves; a filter stays, and hidden rows still count as positions.
...
- **Discovery Mix and "play artist top songs" no longer cancel other work in flight** — the track that was just starting, a download, the auto-advance at the end of a track, the rest of a long playlist being fetched for the queue, or a playback-status update.
- **Start Radio and Add to Library no longer cut a long playlist short** — on the Library page and on playlist pages the remaining tracks keep loading and the "(loading N total…)" subtitle resolves.
- **Search: typing while a search is running no longer cancels it.**
- **Browse › Charts** — a region change interrupted by a tab switch is fetched when you come back to Charts; a chart that failed to load, or whose tracks could not be fetched, can be retried by selecting the tab again instead of showing an empty table; pill clicks during a region change are ignored instead of racing it; the country label can no longer show a country other than the one the data is for.
...
- **Malformed `session.json` and `update_check.json` are ignored** — a file that is valid JSON but not an object raised at startup or in the update check; both are now treated as absent.
- **Saving settings preserves multiline and control characters** — valid string values could produce invalid TOML and reset unrelated settings on the next load. Strings and string-list values now round-trip without losing their contents.
- **Chart locales use the country component** — values such as `en_GB`, `en-GB`, and `es-MX` now select `GB`, `GB`, and `MX` instead of treating the language as a country.
