...
- **Playback no longer 403s on Google's video servers rejecting the Range request mpv makes to actually stream a track** — client selection went through two iterations on this branch. It first moved from `android` (silently dropped by yt-dlp once cookies are present, and not actually relevant to the "made for kids" content it was chosen for — see project notes) to `web_safari`/`web_embedded`, both of which support cookie-based auth; per @Villoh's initial PR #136 testing this hit 24/24 (100%) success vs. 4/12 (33%) for the prior `android`-based selection. Later, more rigorous real-world testing (@Villoh, independently reproduced here against a 25-track sample of real play history using direct HTTP Range-request probing — the actual access pattern mpv uses to stream, which yt-dlp's own downloader does not reproduce and so cannot catch) found `web_safari`/`web_embedded` erratic in practice despite resolving formats without error: GVS immediately 403s the Range request mpv issues (1/25 succeeded here; @Villoh separately measured 16/30). Client selection is now `tv_simply`+`tv_downgraded` (matching PR #137's fix, independently validated here): `tv_downgraded` alone fails extraction outright ("The page needs to be reloaded"), but paired with `tv_simply` — which contributes no formats of its own — resolves and plays back reliably (25/25 in the same sample). yt-dlp's own default client selection — which could otherwise fall back to `android_vr` — is never used now that the client list is always explicit, for every setup type. Separately, stream resolution now reads the cookie data `ytm setup` (browser-based *or* manual header paste — both are parsed into a stream cookiejar the same way) or its automatic session refresh already produced, instead of independently detecting a browser itself. `tv_simply` doesn't support cookie-based auth (yt-dlp's own client metadata), so once a user is authenticated (real session cookies from `ytm setup`) it's silently dropped from the client list, leaving `tv_downgraded` alone — this does *not* reproduce the unauthenticated `tv_downgraded`-alone failure, because yt-dlp separately auto-appends a `web_music` client for any authenticated `music.youtube.com` request, which resolves successfully on its own. Confirmed against a real authenticated session, not a fabricated one — a fabricated session can't be used to test this because the actual player-response/format requests would be rejected by Google using invalid credentials, not because the local client-selection filter is skipped.
- **Stream format selection is now a single fixed selector, not three quality tiers, and includes `bestaudio` again** — `tv_simply`/`tv_downgraded` never exposes more than one non-storyboard combined format per video for an unauthenticated resolve (confirmed empirically: itag 18, a progressive MP4 with AAC audio — no HLS ladder, no separate audio-only rendition to choose from), so the `high`/`medium`/`low` `QUALITY_FORMATS` tiers this branch previously carried (along with the `[playback] audio_quality` setting controlling them) have been removed entirely — there was nothing left for them to differentiate at that layer. The format selector is now `bestaudio/best[vcodec!=none][acodec!=none]`: for an authenticated resolve, the auto-appended `web_music` client (see above) exposes genuine audio-only formats up to ~280kbps opus — a real improvement over itag 18's ~128kbps AAC muxed with a never-rendered video track (`video=False`) — and this branch previously excluded `bestaudio` entirely on the finding that it's "PO-Token-gated regardless of client or cookies," which was never tested against `web_music` specifically. Reinstated after validating 18/18 success on real, distinct tracks from actual play history under a YouTube Music **Premium** account, each probed with two HTTP Range requests (immediate and 2MB into the file, to rule out a delayed-failure pattern too). Premium accounts are exempt from `web_music`'s PO-Token requirement (yt-dlp's own `not_required_for_premium` policy flag) — that policy is otherwise identical to `web_safari`/`web_embedded`'s (the pair proven erratic above), so this validation does **not** cover non-Premium authenticated accounts, which may see the same intermittent GVS-403-after-resolution failures. For an unauthenticated resolve, `bestaudio` matches nothing (no audio-only format is exposed) and the selector correctly falls through to the combined alternative, unchanged from the 25/25-validated behavior above.
...
- **Two concurrency bugs in stream resolution, both confirmed in practice**: a UI freeze from closing a live `YoutubeDL` instance out from under an in-flight resolve; and a reset landing while an options-build was in flight being silently lost, permanently caching an instance built with stale (pre-reset) settings. The generation-check guard for the second case remains in place as defense-in-depth even though its originally-confirmed trigger — a slow, Keychain-backed cookie decryption inside the options build — no longer occurs on this path now that cookie extraction happens solely in `AuthManager`.
- **yt-dlp's own diagnostics are no longer silently invisible** — yt-dlp's own diagnostic output during stream resolution now reaches `ytm.log` instead of being dropped by yt-dlp's default logger.
- **Streaming no longer decrypts your browser's cookies on its own** — stream resolution now reads the same cookie data `ytm setup` (browser-based or manual header paste) or its automatic session-refresh already produced, instead of independently re-decrypting the browser a second time. If setup hasn't run yet, or a session refresh failed to produce a cookie file, streaming proceeds without cookies rather than triggering a decryption you didn't explicitly ask for.
- **The "setting up playback — decrypting browser cookies" toast is gone** — there's no longer a slow decryption at play time for it to explain; the one-time cost (if any) now happens during `ytm setup` itself.
- **The stream resolver still warms up in the background at startup** — using the resume target, current queue track, recent local history, or the YT Music home feed as a fallback (in that priority order) — so the first stream URL is already resolved and cached by the time playback starts, instead of resolving cold on first play.
- **The stream-cookie file is now written by the same component that manages `auth.json`** (`ytm setup`/session refresh), using the same symlink-safe `O_NOFOLLOW` open, scoped to `youtube.com`/`google.com` cookies only instead of the full browser cookie jar (every site the browser has a cookie for).
